...
export default function UpdateCaseComponent({
  ...
  const [tags, setTags] = useState<Option[]>(
    caseData.tags.map((tag) => ({ value: tag, label: tag }))
  );
  const [searchEmail, setSearchEmail] = useState("");
  const [invitedUsers, setInvitedUsers] = useState<User[]>(users);
  ...
  const handleSubmit = async () => {
    ...
    try {
      await updateCase(caseData.id, updatedCase);
    ...
    <div className="flex flex-col gap-12 py-12">
      <Card className="w-full max-w-2xl mx-auto">
        <CardHeader>
          ...
          <CardDescription>Fill in the details to update case</CardDescription>
        </CardHeader>
